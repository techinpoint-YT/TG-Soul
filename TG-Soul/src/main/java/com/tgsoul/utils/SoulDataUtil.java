...
public class SoulDataUtil {
    ...
    public void saveAllPlayerData(Map<UUID, PlayerSoulData> playerData,
        ...
        // Save synchronously if plugin is being disabled
        saveToFile(dataConfig, dataFile, !plugin.isEnabled());
    }
    ...
    /**
     * Saves configuration to file synchronously during shutdown, asynchronously otherwise
     */
    public void saveToFile(FileConfiguration dataConfig, File dataFile) {
        saveToFile(dataConfig, dataFile, false);
    }

    /**
     * Saves configuration to file
     * @param sync true to save synchronously (for shutdown), false for async
     */
    public void saveToFile(FileConfiguration dataConfig, File dataFile, boolean sync) {
        if (sync || !plugin.isEnabled()) {
            ...
            // Only schedule async if plugin is enabled and server is running
            if (plugin.isEnabled() && !plugin.getServer().isStopping()) {
                FileConfiguration configToSave = dataConfig;
                ...
                // Fallback to synchronous save if server is stopping
                try {
                    dataConfig.save(dataFile);
                    plugin.getLogger().info("Player data saved to playerdata.yml successfully (sync fallback).");
                } catch (IOException e) {
                    plugin.getLogger().severe("Could not save player data to playerdata.yml: " + e.getMessage());
                    e.printStackTrace();
                }
