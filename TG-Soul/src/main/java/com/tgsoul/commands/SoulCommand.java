...
public class SoulCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        return switch (subCommand) {
            ...
            case "resourcepack" -> handleResourcePack(sender, args);
            case "testparticle" -> handleTestParticle(sender, args);
            default -> {
                handleHelp(sender);
                yield false;
            }
    ...
    private boolean handleHelp(CommandSender sender) {
        ...
        if (sender.hasPermission("tgsoul.admin")) {
            ...
            plugin.getMessageUtil().sendMessageWithoutPrefix(sender, "help-admin-testparticle");
        }
        return true;
    }
    ...
    private boolean handleReload(CommandSender sender) {
        ...
        try {
            ...
            // Refresh SoulBarManager with new CustomModelData
            plugin.getSoulBarManager().refreshCustomModelData();
            // Update all soul bars for online players
            plugin.getSoulBarManager().updateAllSoulBars();
    ...
    private boolean handleTestParticle(CommandSender sender, String[] args) {
        if (!sender.hasPermission("tgsoul.admin")) {
            plugin.getMessageUtil().sendMessage(sender, "no-permission");
            return true;
        }
        ...
        if (args.length < 2) {
            sender.sendMessage("§eUsage: /soul testparticle <lose|gain|both>");
            return true;
        }
        ...
        switch (type) {
            case "lose" -> {
                player.sendMessage("§eTesting lose particle effect...");
                plugin.getParticleManager().playLoseEffect(player);
            }
            case "gain" -> {
                player.sendMessage("§eTesting gain particle effect...");
                plugin.getParticleManager().playGainEffect(player);
            }
            case "both" -> {
                player.sendMessage("§eTesting both particle effects...");
                plugin.getParticleManager().testParticles(player);
            }
            default -> {
                player.sendMessage("§cInvalid argument! Use: lose, gain, or both");
                return true;
            }
        }
    ...
    private boolean handleResourcePack(CommandSender sender, String[] args) {
        ...
        if (!plugin.getConfigManager().isResourcePackEnabled()) {
            player.sendMessage("§cResource pack system is disabled.");
            return true;
        }

        String url = plugin.getConfigManager().getResourcePackUrl();
        if (url.isEmpty()) {
            player.sendMessage("§cNo resource pack URL configured. Please contact an administrator.");
            return true;
        }

        String hash = plugin.getConfigManager().getResourcePackHash();
        String prompt = plugin.getConfigManager().getResourcePackPrompt().replace("\\n", "\n");
        ...
        try {
            ...
            player.sendMessage("§aSending resource pack... Please accept the download prompt.");
        } catch (Exception e) {
            player.sendMessage("§cFailed to send resource pack: " + e.getMessage());
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            List<String> subCommands = new ArrayList<>(Arrays.asList("help", "recipe", "give", "take", "set", "get", "unban", "reload", "top", "stats", "pack", "resourcepack"));
            if (sender.hasPermission("tgsoul.admin")) {
                subCommands.add("testparticle");
            }
        ...
        } else if (args.length == 2 && args[0].equalsIgnoreCase("testparticle") && sender.hasPermission("tgsoul.admin")) {
            List<String> types = Arrays.asList("lose", "gain", "both");
